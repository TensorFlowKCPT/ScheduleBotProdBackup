...
''', (ChatId, group_id))
    ...
    @staticmethod
    def RegPrepod(ChatId):
        with sqlite3.connect('ScheduleBot.db') as conn:
            c = conn.cursor()
            # Добавляем нового пользователя в таблицу PrepodUsers
            c.execute('''
    INSERT INTO PrepodUsers (id)
    VALUES (?)
''', (ChatId))
            conn.commit()
    ...
    def is_user_prepod(user_id):
        with sqlite3.connect('ScheduleBot.db') as conn:
            cursor = conn.execute("SELECT EXISTS(SELECT 1 FROM PrepodUsers WHERE id = ?)", (user_id,))
            result = cursor.fetchone()
            return bool(result)
    @staticmethod
    ...
    def StartDatabase():
        ...
        with sqlite3.connect('ScheduleBot.db') as conn:
            conn.execute('CREATE TABLE IF NOT EXISTS PrepodUsers (id INTEGER PRIMARY KEY)')
